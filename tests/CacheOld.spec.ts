...
describe('Cache', () => {
  ...
  it('should be constructable via new', () => {
    const cache = new OldCache({garbageCollectInterval: 0});

    expect(cache instanceof OldCache).toBe(true);
  });

  it('should contain entries passed to constructor', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({getSeconds, garbageCollectInterval: 0}, [
      [
        'someKey1',
        {
          k: 'someKey1',
  ...
  describe('finite capacity cache', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({capacity: 5, getSeconds, garbageCollectInterval: 0});
  ...
  describe('infinite capacity cache', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({capacity: Infinity, getSeconds, garbageCollectInterval: 0});
  ...
  describe('coning cache', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({capacity: 5, getSeconds, clone: true, garbageCollectInterval: 0});
  ...
  describe('no-coning cache', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({capacity: 5, getSeconds, clone: false, garbageCollectInterval: 0});
  ...
  describe('garbage collector', () => {
    const getSeconds = getControllableSecondsFetcher();
    const cache = new OldCache({capacity: 5, getSeconds, garbageCollectInterval: 60});
    const cache2 = new OldCache({capacity: 5, getSeconds, garbageCollectInterval: 0});
    const cache3 = new OldCache({capacity: 200, getSeconds, garbageCollectInterval: 60});
