...
export default class TaggedCache {
    ...
    public get(key: TaggedCacheKey, defaultValue?: any, raw?: true): TaggedCacheEntry;
    public get(key: TaggedCacheKey, defaultValue: any = undefined, raw: boolean = false): any | TaggedCacheEntry {
    ...
    public mget(keys: TaggedCacheKeysList, defaultValue?: any, raw?: false): MultiGetResult;
    public mget(keys: TaggedCacheKeysList, defaultValue?: any, raw?: true): MultiGetEntriesResult;
    public mget(
    ...
    public validate(entry: TaggedCacheEntry, now: number = 0): boolean {
        if ((entry.exp && (now || timestamp()) >= entry.exp) || (entry.tags && !this.tags.validate(entry.tags))) {
